// Note #1: Cake.CsvHelper The loaddependencies=true will take effect 
// in case using Cake built in NuGet support and not the external nuget.exe
//
// Note #2: Cake.CsvHelper We must use an older version because the newer version is incompatible with the referenced CsvHelper .NET package
// (references CsvClassMap which is renamed to ClassMap)
...
var workFolder = Argument("work-folder", "C:/TempRepos/"); // Default should be "C:/Source/"
var gitUserName = Argument("git-username", "<username>");
var gitPassword = Argument("git-password", "******");
var repositoryCsv = Argument("repository-csv", "repositories.csv");
...
Task("Default")
    .Does(() =>
{
    Information("Repository Cloner v0.9");

    var repositoryInfos = ReadCsv<RepositoryInfo>(repositoryCsv, new CsvHelperSettings { HasHeaderRecord = true });
    // Only for testing:
    // repositoryInfos = new [] {
    //     new RepositoryInfo {CloneUrl = "https://github.com/pluraltouch/example.git", GitUrl = "" }
    // };
    foreach(var info in repositoryInfos)
    {
        ...
        if (!DirectoryExists(repositoryFolder))
        {
            try
            {
                ...
                Information($"Successfully cloned '{info.CloneUrl}' to '{repositoryFolder}'");
            }
            catch(Exception e)
            {
